...
func Unlock(device string, passphrase []byte, name string) error {
	...
	if err := ensureDeviceNode(name); err != nil {
...
func Lock(name string) error {
	...
	if info != nil {
		...
		dmPath := fmt.Sprintf("/dev/dm-%d", minor)
		os.Remove(dmPath) // Ignore error - may already be gone
	}
	mapperPath := fmt.Sprintf("/dev/mapper/%s", name)
	os.Remove(mapperPath) // Ignore error - may already be gone
...
func IsUnlocked(name string) bool {
	...
	// Fall back to checking dmsetup directly (for containers without udev)
